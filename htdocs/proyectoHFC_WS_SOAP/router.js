  ...
  MyService: {
      MyPort: {
          ...
          Authentication: function(args, cb, headers, req) {
              ...
              const contentype = (req.headers['content-type'] == '' || req.headers['content-type'] !== 'text/xml')? '' : req.headers['content-type'];
              ...
              if(contentype === '' || nombreUsuario === '' || passwordUsuario === ''){
                  ...
                  message = (contentype !== 'text/xml' && contentype === '') ? 'ERROR 202026. DEBE ENVIAR EL CONTENT-TYPE DE MANERA OBLIGATORIA.' : message;
          ...
          getInfoBasicaServicioHFCxCliente: function(args, cb, headers, req) {
            ...
            const contentype = (req.headers['content-type'] == '' || req.headers['content-type'] !== 'text/xml')? '' : req.headers['content-type'];
            const nombreUsuario = (headers.Security.UsernameToken.Username == '' || headers.Security.UsernameToken.Username == 0)? '' : headers.Security.UsernameToken.Username;
            const authorization = ( req.headers.authorization == '' || req.headers.authorization == 0 || !(req.headers.authorization) )? '' : req.headers.authorization;
            ...
            if(contentype === '' || token === '' || idCliente === '' || idCliente == 0 || isNaN(idCliente) || nombreUsuario === ''){
          ...
          getPruebasCablemodem: function(args, cb, headers, req) {
            ...
            if(contentype === '' || token === '' || nombreUsuario === '' || mac_address === '' || mac_address == 0){
          ...
          getPruebasCablemodemIW: function(args, cb, headers, req) {
            ...
            if(contentype === '' || token === '' || nombreUsuario === '' || mac_address === '' || mac_address == 0 || idservicio === '' || idservicio == 0 || isNaN(idservicio) || idproducto === '' || idproducto == 0 || isNaN(idproducto)){
